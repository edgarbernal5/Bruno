...
namespace BrunoFX
{
	...
	bool HLSLGenerator::Generate(HLSLTree* tree, Target target, const char* entryName, bool legacy)
	{
		...
		if (!legacy)
		{
			...
			if (function->returnType.baseType == HLSLBaseType_UserDefined)
			{
				...
				HLSLStructField * sv_fields = nullptr;
				...
				while (field) {
					...
					if (field->semantic) {
						...
						if (field->sv_semantic != nullptr) {
							// Unlink from last.
							if (lastField != nullptr) lastField->nextField = nextField;
				...
				if (sv_fields != nullptr) {
					if (lastField == nullptr) {
						s->field = sv_fields;
					}
					else {
						ASSERT(lastField->nextField == nullptr);
	...
	void HLSLGenerator::OutputExpression(HLSLExpression* expression)
	{
		...
		else if (expression->nodeType == HLSLNodeType_FunctionAccess)
		{
			/*
			HLSLMemberAccess* memberAccess = static_cast<HLSLMemberAccess*>(expression);
			m_writer.Write("(");
			...
			*/
			...
			HLSLFunctionAccess* functionCall = static_cast<HLSLFunctionAccess*>(expression);
			OutputExpression(functionCall->object);
			const char* name = functionCall->functionName;
	...
	void HLSLGenerator::OutputArguments(HLSLArgument* argument)
	{
		...
		while (argument != nullptr)
		{
			...
			OutputDeclaration(argument->type, argument->name, semantic, /*registerName=*/nullptr, argument->defaultValue);
			argument = argument->nextArgument;
			++numArgs;
		}
	}
		...
		while (statement != nullptr)
		{
			...
			else if (statement->nodeType == HLSLNodeType_Buffer)
			{
				...
				while (field != nullptr)
				{
					...
					{
						m_writer.BeginLine(indent + 1, field->fileName, field->line);
						OutputDeclaration(field->type, field->name, /*semantic=*/nullptr, /*registerName*/field->registerName, field->assignment);
						m_writer.Write(";");
						m_writer.EndLine();
					}
			...
			else if (statement->nodeType == HLSLNodeType_Technique11 || statement->nodeType == HLSLNodeType_Pass11 || statement->nodeType == HLSLNodeType_CompilerShader || statement->nodeType == HLSLNodeType_PassShader)
			{
				// Techniques are ignored.
			}
			else
	...
	void HLSLGenerator::OutputDeclarationBody(const HLSLType& type, const char* name, const char* semantic/*=nullptr*/, const char* registerName/*=nullptr*/, HLSLExpression * assignment/*=nullptr*/)
	{
		m_writer.Write("%s", name);
		...
		if (assignment != nullptr && !IsSamplerType(type))
		{
			m_writer.Write(" = ");
			if (type.array)
			{
	...
	void HLSLGenerator::OutputDeclaration(const HLSLType& type, const char* name, const char* semantic/*=nullptr*/, const char* registerName/*=nullptr*/, HLSLExpression * assignment/*=nullptr*/)
	{
