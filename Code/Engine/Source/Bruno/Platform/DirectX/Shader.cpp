...
namespace Bruno
{
    ...
    void Shader::AddProgram(ShaderProgram&& program)
    {
        ...
        if (program.GetStage() == ShaderStage::Vertex) 
            m_vertexProgram = std::make_unique<ShaderProgram>(std::move(program));
        else if (program.GetStage() == ShaderStage::Pixel)
            m_pixelProgram = std::make_unique<ShaderProgram>(std::move(program));
    }
}
