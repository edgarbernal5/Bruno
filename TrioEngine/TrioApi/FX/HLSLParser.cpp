...
#define SAMPLER_INTRINSIC_FUNCTION(name, sampler, arg1) \
	...
	static uint32_t GetTypeSizeInBytes(const HLSLType& type)
	{
		if (type.baseType == HLSLBaseType_UserDefined)
		{
			return 0;
		...
		{
			const BaseTypeDescription desc = _baseTypeDescriptions[type.baseType];
			int componentSizeInBytes = 0;
			
			switch (desc.numericType)
			{
			...
			return (uint32_t)(desc.numComponents * desc.height * componentSizeInBytes);
		}
	}
	...
	*/
	...
	HLSLParser::HLSLParser(TrioMem::Allocator* allocator, const char* fileName, const char* buffer, size_t length) :
	...
	bool HLSLParser::Expect(int token)
	{
		if (!Accept(token))
		{
			...
			char near_s[HLSLTokenizer::s_maxIdentifier];
			m_tokenizer.GetTokenName(near_s);
			m_tokenizer.Error("Syntax error: expected '%s' near '%s'", want, near_s);
			return false;
		}
		return true;
	}
	...
	bool HLSLParser::Expect(const char * token)
	{
		if (!Accept(token))
		{
			...
			char near_s[HLSLTokenizer::s_maxIdentifier];
			m_tokenizer.GetTokenName(near_s);
			m_tokenizer.Error("Syntax error: expected '%s' near_s '%s'", want, near_s);
			return false;
		}
		return true;
	}
	...
	bool HLSLParser::ExpectIdentifier(const char*& identifier)
	{
		if (!AcceptIdentifier(identifier))
		{
			char near_s[HLSLTokenizer::s_maxIdentifier];
			m_tokenizer.GetTokenName(near_s);
			m_tokenizer.Error("Syntax error: expected identifier near '%s'", near_s);
			identifier = "";
			return false;
		}
		return true;
	...
	bool HLSLParser::ParseTopLevel(HLSLStatement*& statement)
	{
		...
		else if (Accept(HLSLToken_CBuffer) || Accept(HLSLToken_TBuffer))
		{
			...
			while (!Accept('}'))
			{
				...
				CheckAndDeclareVariable(field->name, field->type);
		...
		{
			/*HLSLBaseType typeTemplate;
			const char*  typeNameTemplate = nullptr;
			int          typeFlagsTemplate = 0;*/

			//Texture2D<float> ?? @@Edgar
			//if (Accept('<'))
			//{
			//	if (!ExpectType(false, typeTemplate, typeNameTemplate, &typeFlagsTemplate) || !Expect('>'))
			//	{
			//		return false;
			//	}
			//}
			...
			else
			{
				...
				//{
				//	declaration->templateName = typeNameTemplate;
				//	declaration->templateType.baseType = typeTemplate;
				//	declaration->templateType.flags = typeFlagsTemplate;
				//}
		...
		else if (ParseTechnique11(statement)) {
			doesNotExpectSemicolon = true;
		}
	...
	//bool HLSLParser::ParseBufferFieldDeclaration(HLSLBufferField*& declaration)
	//{
	//	const char* fileName = GetFileName();
	//	int         line = GetLineNumber();
	...
	//	HLSLBufferField * firstDeclaration = nullptr;
	//	HLSLBufferField * lastDeclaration = nullptr;
	...
	//	HLSLBufferField * fieldDeclaration = m_tree->AddNode<HLSLBufferField>(fileName, line);
	//	fieldDeclaration->type = type;
	//	fieldDeclaration->name = name;

	//	DeclareVariable(fieldDeclaration->name, fieldDeclaration->type);
	...
	//	if (firstDeclaration == nullptr) firstDeclaration = fieldDeclaration;
	//	if (lastDeclaration != nullptr) lastDeclaration->nextField = fieldDeclaration;
	//	lastDeclaration = fieldDeclaration;
	...
	bool HLSLParser::ParseTerminalExpression(HLSLExpression*& expression, bool& needsEndParen)
	{
		...
		while (!done)
		{
			...
			while (Accept('.'))
			{
				if (expression->expressionType.baseType == HLSLBaseType_Texture2D)
				{
					HLSLFunctionAccess* functionAccess = m_tree->AddNode<HLSLFunctionAccess>(fileName, line);
					functionAccess->object = expression;
					done = false;
					if (!ExpectIdentifier(functionAccess->functionName))
					{
						return false;
					}
					...
					if (!ParseExpressionList(')', false, functionAccess->argument, functionAccess->numArguments))
					{
						return false;
					}
	...
	{
		if (!Expect(HLSLToken_sampler_state))
		{
			return false;
		}
	...
	bool HLSLParser::ParseTechnique11(HLSLStatement*& statement)
	{
		if (!Accept(HLSLToken_Technique11)) {
			return false;
		}
		...
		HLSLTechnique11* technique = m_tree->AddNode<HLSLTechnique11>(GetFileName(), GetLineNumber());
		technique->name = techniqueName;

		//m_techniques11.push_back(technique);

		HLSLPass11* lastPass = nullptr;
	...
	bool HLSLParser::ParsePass11(HLSLPass11*& pass)
	{
		if (!Accept(HLSLToken_Pass)) {
			return false;
		}
		...
		pass = m_tree->AddNode<HLSLPass11>(fileName, line);
		pass->name = passName;

		HLSLPassShader* lastpassShader = nullptr;
		...
		while (!Accept('}'))
		{
			...
			HLSLPassShader* passShader = nullptr;
			if (!ParsePassShader(passShader))
			{
				return false;
			}
			if (lastpassShader == nullptr)
			{
				pass->shader = passShader;
			}
			else
			{
				lastpassShader->nextShader = passShader;
			}
			lastpassShader = passShader;
			pass->numPassShader++;
		}
		return true;
	}

	bool HLSLParser::ParsePassShader(HLSLPassShader*& statement)
	{
		const char* shader;
		if (Accept(HLSLToken_SetVertexShader))
		{
			shader = "VertexShader";
		}
		else if (Accept(HLSLToken_SetPixelShader))
		{
			shader = "PixelShader";
		}
		else if (Accept(HLSLToken_SetGeometryShader))
		{
		...
		statement = m_tree->AddNode<HLSLPassShader>(fileName, line);
		statement->name = shader;
		...
		if (!Expect(HLSLToken_CompileShader) || !Expect('('))
		{
			return false;
		}

		HLSLCompilerShader* compiler = m_tree->AddNode<HLSLCompilerShader>(fileName, line);
		statement->options = compiler;
		
		const char* compilerVersion = nullptr;
		const char* entryFunction = nullptr;
		ExpectIdentifier(compilerVersion);

		compiler->profile = compilerVersion;
		...
		if (!ExpectIdentifier(entryFunction) || !Expect('('))
		{
			return false;
		}
		HLSLFunctionCall* functionCall = m_tree->AddNode<HLSLFunctionCall>(fileName, line);
		compiler->functionCall = functionCall;
		compiler->functionCallName = entryFunction;
		...
		//const HLSLFunction* function = MatchFunctionCall(functionCall, entryFunction);
	...
	bool HLSLParser::AcceptType(bool allowVoid, HLSLType& type)
	{
		{
			type.flags = 0;
