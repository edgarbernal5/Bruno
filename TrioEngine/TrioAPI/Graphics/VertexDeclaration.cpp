...
#ifdef TRIO_DIRECTX
	std::vector<D3D11_INPUT_ELEMENT_DESC> VertexDeclaration::GetInputElements()
	{
		std::vector<D3D11_INPUT_ELEMENT_DESC> inputs(m_vElements.size());
		for (int i = 0; i < m_vElements.size(); i++)
		{
			inputs[i] = m_vElements[i].GetInputElement();
		}
		return inputs;
	}
